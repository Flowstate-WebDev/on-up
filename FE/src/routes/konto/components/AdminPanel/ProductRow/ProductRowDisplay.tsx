...
export function ProductRowDisplay({ product, onEdit, onDelete }: ProductRowDisplayProps) {
    ...
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-24 h-24 bg-bg-secondary rounded-xl overflow-hidden border border-border-secondary flex items-center justify-center shrink-0">
        {product.imageUrl ? (
          <img 
            ...
            onError={(e) => {
              (e.target as HTMLImageElement).src = 'https://via.placeholder.com/150?text=Brak+foto'
            }}
          />
        ) : (
          <span className="text-text-tertiary text-[10px] text-center p-2 uppercase font-bold">Brak foto</span>
        )}
      </div>
      ...
      <div className="flex-1 space-y-3">
        <div className="flex justify-between">
          <div>
            <h3 className="text-lg font-semibold text-text-primary leading-tight">{product.title}</h3>
            <div className="flex flex-wrap gap-2 mt-2">
              {product.professions?.map((p: any) => (
                <span key={p.professionId} className="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-bold rounded-md uppercase tracking-wider">
                  {p.profession.name}
                </span>
              ))}
              {product.qualifications?.map((q: any) => (
                <span key={q.qualificationId} className="px-2 py-0.5 bg-secondary-light/10 text-text-tertiary text-[10px] font-bold rounded-md uppercase tracking-wider border border-border-secondary">
          ...
          <div className="text-right shrink-0">
            ...
            <p className="text-[10px] font-bold text-text-tertiary uppercase">Stan: {product.stock}</p>
          </div>
        </div>
