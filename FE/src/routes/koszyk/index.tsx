...
function CartPage() {
  const { products, groupedItems, totalPrice, isStockIssue, removeFromCart } =
    useCart();
  ...
  if (products.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Link
          to="/"
          className="bg-primary hover:bg-secondary text-text-obj px-6 py-2 rounded-lg transition-colors"
        >
          Przejdź do sklepu
        </Link>
      </div>
