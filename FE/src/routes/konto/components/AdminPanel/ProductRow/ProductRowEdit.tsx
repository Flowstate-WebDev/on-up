    ...
    <div className="flex flex-col md:flex-row gap-6">
      ...
      <div className="w-full md:w-24 h-24 bg-bg-secondary rounded-xl overflow-hidden border border-border-secondary flex items-center justify-center shrink-0">
        {formData.imageUrl ? (
          ...
          <span className="text-text-tertiary text-[10px] text-center p-2 uppercase font-bold">Preview</span>
        )}
      </div>
      ...
      <div className="flex-1 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <Input name="title" value={formData.title} onChange={onChange} placeholder="Tytuł książki" />
          </div>
          <div className="space-y-1">
            <AdminLabel>Cena (zł)</AdminLabel>
            <Input type="number" step="0.01" name="price" value={formData.price} onChange={onChange} placeholder="0.00" />
          </div>
          <div className="space-y-1">
            <AdminLabel>Stan magazynowy</AdminLabel>
            <Input type="number" name="stock" value={formData.stock} onChange={onChange} placeholder="0" />
          </div>
          <div className="space-y-1">
            <AdminLabel>Plik obrazu (np. Photo.webp)</AdminLabel>
            <Input name="imageUrl" value={formData.imageUrl} onChange={onChange} placeholder="image.webp" />
          </div>
        </div>
