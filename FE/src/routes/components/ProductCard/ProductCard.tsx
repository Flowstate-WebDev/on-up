...
export const ProductCard = ({ data }: ProductCardProps) => {
  return (
    <Link
      ...
      className="w-full flex flex-col bg-bg-secondary shadow-md rounded-lg overflow-hidden hover:shadow-lg transition-shadow h-full relative"
    >
      ...
      <div className="p-3 flex flex-col flex-1 gap-y-3">
        <h2 className="font-bold leading-tight line-clamp-3 overflow-hidden text-sm sm:text-base min-h-[3em]">
          {data.title}
        </h2>
        <div className="flex-1">
          <h3 className="text-[10px] uppercase tracking-wider text-text-tertiary mb-1">
            Kwalifikacje:
          </h3>
          <ul className="flex flex-wrap gap-1">
            {data.qualifications.length > 0 ? (
              data.qualifications.map((item, idx) => (
                <li key={idx}>
                  <Tag>{item.qualification.code}</Tag>
                </li>
              ))
            ) : (
